...
const COOKIE = "cryptonichub_session";
const ADMIN_COOKIE = "cryptonichub_admin";
const DAY = 60 * 60 * 24;
...
function secret(): Uint8Array {
  ...
  const url = dbUrl();
  if (url) {
    const derived = createHash("sha256").update("cryptonichub::" + url).digest("hex");
...
export async function currentUser(): Promise<{
  ...
  const rows = (await sql`
    SELECT id, email, name, role, balance, country FROM cryptonichub_users WHERE id = ${s.id} LIMIT 1
  `) as Array<{
    id: number;
    email: string;
    name: string;
