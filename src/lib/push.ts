...
// Optional:                  VAPID_SUBJECT (default mailto:support@cryptonichub.tech)
...
function ensureVapid(): boolean {
  ...
  if (!configured) {
    webpush.setVapidDetails(
      process.env.VAPID_SUBJECT || "mailto:support@cryptonichub.tech",
...
export async function sendPushToUser(userId: number, payload: PushPayload): Promise<void> {
  ...
  try {
    ...
    const subs = (await sql`
      SELECT endpoint, p256dh, auth FROM cryptonichub_push_subs WHERE user_id = ${userId}
      ...
      subs.map(async (s) => {
        try {
          ...
          if (e?.statusCode === 404 || e?.statusCode === 410) {
            await sql`DELETE FROM cryptonichub_push_subs WHERE endpoint = ${s.endpoint}`.catch(() => {});
          }
        }
      })
    );
