...
// Signup phone-OTP. Codes are stored HASHED in cryptonichub_otps (never
// plaintext), keyed by the normalized phone. 6 digits, 10-min TTL, 60s resend
// cooldown, 5-attempt lock. Enforced only for the countries in
// OTP_SIGNUP_COUNTRIES (default "KE") AND only when SMS is actually configured —
// so a missing SMS setup never blocks signups.
...
export async function sendSignupOtp(
  ...
  const existing = (await sql`
    SELECT last_sent_at FROM cryptonichub_otps WHERE phone = ${phone} LIMIT 1
  ...
  await sql`
    INSERT INTO cryptonichub_otps (phone, code_hash, expires_at, attempts, last_sent_at)
...
export async function verifySignupOtp(
  ...
  const rows = (await sql`
    SELECT code_hash, expires_at, attempts FROM cryptonichub_otps WHERE phone = ${phone} LIMIT 1
  ...
  if (hashCode(phone, clean) !== otp.code_hash) {
    await sql`UPDATE cryptonichub_otps SET attempts = attempts + 1 WHERE phone = ${phone}`;
    return { ok: false, error: "Incorrect code. Try again." };
  }

  // Correct — consume it so it can't be reused.
  await sql`DELETE FROM cryptonichub_otps WHERE phone = ${phone}`;
  return { ok: true };
}
