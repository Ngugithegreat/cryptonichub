...
// Optional env:  EMAIL_FROM   (default "Cryptonichub <noreply@cryptonichub.tech>")
//                PUBLIC_BASE_URL (used to build links; default cryptonichub.tech)
//
...
export function emailFrom(): string {
  return (process.env.EMAIL_FROM || "Cryptonichub <noreply@cryptonichub.tech>").trim();
}

export function siteUrl(): string {
  const base = process.env.PUBLIC_BASE_URL || "https://cryptonichub.tech";
  return base.replace(/\/$/, "");
}
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #e6e8ee;border-radius:16px;overflow:hidden;">
    <tr><td style="padding:22px 28px;border-bottom:1px solid #eef0f4;">
      <table role="presentation" cellpadding="0" cellspacing="0"><tr>
        <td style="vertical-align:middle;"><img src="${siteUrl()}/apple-icon" width="32" height="32" alt="Cryptonichub" style="display:block;border-radius:8px;"></td>
        <td style="vertical-align:middle;padding-left:10px;font-size:18px;font-weight:700;color:#0f1116;letter-spacing:-0.02em;">Cryptonichub</td>
      </tr></table>
    </td></tr>
    ...
    <tr><td style="padding:16px 28px;border-top:1px solid #eef0f4;color:#8a90a0;font-size:12px;line-height:1.6;">
      Cryptonichub · <a href="${siteUrl()}" style="color:#2F6FED;text-decoration:none;">cryptonichub.tech</a><br>
...
export function welcomeEmail(name: string): { subject: string; html: string; text: string } {
  ...
  const html = shell(
    `<h1 style="margin:0 0 12px;color:#0f1116;font-size:22px;">Welcome to Cryptonichub, ${first} 👋</h1>
  ...
  const text = `Welcome to Cryptonichub, ${first}! Your account is live. Start trading at ${url}`;
  return { subject: "Welcome to Cryptonichub 🎉", html, text };
}
...
export function depositReceiptEmail(
  ...
  const html = shell(
    `<h1 style="margin:0 0 12px;color:#0f1116;font-size:22px;">Deposit confirmed ✅</h1>
     <p style="margin:0 0 16px;">Hi ${first}, we've credited <b style="color:#0BA66D;">${amt}</b> to your Cryptonichub balance via ${method}. It's ready to trade.</p>
  ...
  return { subject: `Deposit confirmed — ${amt}`, html, text: `Your Cryptonichub deposit of ${amt} via ${method} is confirmed and ready to trade.` };
}
...
export function withdrawalReceiptEmail(
  ...
  return { subject: `Withdrawal request received — ${amt}`, html, text: `We received your Cryptonichub withdrawal request of ${amt} to ${destination}. Processing now.` };
}
...
export function kycApprovedEmail(name: string): { subject: string; html: string; text: string } {
  ...
  const html = shell(
    `<h1 style="margin:0 0 12px;color:#0f1116;font-size:22px;">You're verified ✅</h1>
     <p style="margin:0 0 16px;">Good news, ${first} — your identity has been verified. You can now withdraw any amount from your Cryptonichub account.</p>
     <p style="margin:0 0 4px;">${button("Go to your wallet", url)}</p>`
  );
  return { subject: "Your Cryptonichub account is verified", html, text: "Your identity has been verified — you can now make large withdrawals." };
}
...
export function kycRejectedEmail(name: string, reason: string): { subject: string; html: string; text: string } {
  ...
  return { subject: "Action needed: verify your Cryptonichub account", html, text: `Your verification was not approved: ${why}` };
}
...
export function referralEarnedEmail(
  ...
  const html = shell(
    `<h1 style="margin:0 0 12px;color:#0f1116;font-size:22px;">You earned a referral reward 🎉</h1>
     <p style="margin:0 0 16px;">Nice one, ${first}! <b>${(friend || "Someone you referred").split(" ")[0]}</b> just made their first deposit, so we've added <b style="color:#0BA66D;">${amt}</b> to your Cryptonichub balance.</p>
  ...
  return { subject: `You earned ${amt} in referral rewards`, html, text: `${friend} made their first deposit — you earned ${amt} on Cryptonichub.` };
}
...
export function verifyEmailOtp(name: string, code: string): { subject: string; html: string; text: string } {
  ...
  const html = shell(
    `<h1 style="margin:0 0 12px;color:#0f1116;font-size:22px;">Confirm your email</h1>
     <p style="margin:0 0 16px;">Hi ${first}, enter this code in Cryptonichub to verify your email address:</p>
     ...
     <p style="margin:0;color:#8a90a0;font-size:13px;">This code expires in 10 minutes. If you didn't create a Cryptonichub account, you can ignore this email.</p>`
  );
  const text = `Your Cryptonichub verification code is ${code}. It expires in 10 minutes.`;
  return { subject: `Your Cryptonichub code: ${code}`, html, text };
}
...
export function resetPasswordEmail(name: string, link: string): { subject: string; html: string; text: string } {
  ...
  const html = shell(
    `<h1 style="margin:0 0 12px;color:#0f1116;font-size:22px;">Reset your password</h1>
     <p style="margin:0 0 16px;">Hi ${first}, we got a request to reset your Cryptonichub password. Click below to choose a new one — this link expires in 1 hour.</p>
     ...
     <p style="margin:0 0 16px;word-break:break-all;color:#2F6FED;font-size:13px;">${link}</p>
  ...
  const text = `Reset your Cryptonichub password (expires in 1 hour): ${link}`;
  return { subject: "Reset your Cryptonichub password", html, text };
}
