...
export async function reconcilePendingTeronaPayouts(userId: number): Promise<void> {
  ...
  const sql = db();
  const pending = (await sql`
    SELECT id, provider_ref, amount, method FROM cryptonichub_transactions
...
export async function reconcilePendingTeronaDeposits(userId: number): Promise<void> {
  ...
  const sql = db();
  const pending = (await sql`
    SELECT provider_ref, method FROM cryptonichub_transactions
...
export async function reconcileAllPendingTeronaPayouts(
  ...
  const sql = db();
  const pending = (await sql`
    SELECT id, user_id, provider_ref, amount, method FROM cryptonichub_transactions
  ...
  for (const w of pending) {
    try {
      ...
      if (isPaid(tp.data.status)) {
        const r = (await sql`
          UPDATE cryptonichub_transactions SET status = 'completed', note = 'Payout completed'
