...
// Runtime, admin-tunable settings kept in the cryptonichub_settings key/value table.
...
export async function getHouseEdge(): Promise<number> {
  ...
  const rows = (await sql`
    SELECT value FROM cryptonichub_settings WHERE key = ${HOUSE_EDGE_KEY} LIMIT 1
...
export async function setHouseEdge(edge: number): Promise<number> {
  ...
  await sql`
    INSERT INTO cryptonichub_settings (key, value, updated_at)
...
async function getIntSetting(key: string, def: number, min: number, max: number): Promise<number> {
  await ensureSchema();
  const sql = db();
  const rows = (await sql`SELECT value FROM cryptonichub_settings WHERE key = ${key} LIMIT 1`) as Array<{ value: string }>;
...
export async function getReferralPct(): Promise<number> {
  ...
  const sql = db();
  const rows = (await sql`
    SELECT value FROM cryptonichub_settings WHERE key = ${REFERRAL_PCT_KEY} LIMIT 1
  `) as Array<{ value: string }>;
  const v = rows.length ? Number(rows[0].value) : NaN;
...
export async function isBlocked(userId: number): Promise<boolean> {
  const sql = db();
  const rows = (await sql`
    SELECT status FROM cryptonichub_users WHERE id = ${userId} LIMIT 1
...
export async function isWithdrawBlocked(
  ...
  const sql = db();
  const rows = (await sql`
    SELECT COALESCE(withdraw_blocked, false) AS b FROM cryptonichub_users WHERE id = ${userId} LIMIT 1
  `) as Array<{ b: boolean }>;
