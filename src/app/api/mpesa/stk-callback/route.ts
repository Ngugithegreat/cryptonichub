...
export async function POST(req: Request) {
  ...
  const found = (await sql`
    SELECT * FROM cryptonichub_transactions
  ...
  if (!verified || !callbackSuccess) {
    ...
    await sql`
      UPDATE cryptonichub_transactions SET status = 'rejected', note = ${
  ...
  if (claimed.length) {
    await sql`UPDATE cryptonichub_users SET balance = balance + ${Number(tx.amount)} WHERE id = ${tx.user_id}`;
  }
