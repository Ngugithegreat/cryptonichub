...
export async function POST(req: Request) {
  ...
  const rows = (await sql`
    SELECT id, user_id FROM cryptonichub_password_resets
  ...
  await sql`UPDATE cryptonichub_users SET password_hash = ${hash} WHERE id = ${user_id}`;
  // Single-use: burn this token and any other outstanding ones for the user.
  await sql`UPDATE cryptonichub_password_resets SET used = true WHERE user_id = ${user_id}`;
  void id;
